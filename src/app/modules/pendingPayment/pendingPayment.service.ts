...
const payCancellationFeeNow = async (
  ...
  const successUrl = `${config.stripe.BASE_URL || "http://62.72.26.31:5005"}/api/v1/stripe/payment/success?session_id={CHECKOUT_SESSION_ID}&type=${pendingPayment.type}&pendingPaymentId=${pendingPaymentId}`;
  const cancelUrl = `${config.stripe.BASE_URL || "http://62.72.26.31:5005"}/api/v1/stripe/payment/cancel?session_id={CHECKOUT_SESSION_ID}&type=${pendingPayment.type}&pendingPaymentId=${pendingPaymentId}`;
