...
const getWalletSummary = catchAsync(async (req: Request, res: Response) => {
  ...
  sendResponse(res, {
    ...
    data: {
      ...
      stripeConnected: !!(user?.stripeCustomerId || user?.stripeConnectedAccountId),
    },
  });
});
...
const getDriverWalletSummary = catchAsync(async (req: Request, res: Response) => {
  const userId = req.user.id;
  const wallet = await WalletService.getOrCreateWallet(userId);
  ...
  totalEarningsQuery.$or = [{ userId: new Types.ObjectId(userId) }, { driverId: driver._id }];
  ...
  pendingQuery.$or = [{ userId: new Types.ObjectId(userId) }, { driverId: driver._id }];
...
const getDriverTransactionHistory = catchAsync(async (req: Request, res: Response) => {
  const userId = req.user.id;
  const role = "driver";
