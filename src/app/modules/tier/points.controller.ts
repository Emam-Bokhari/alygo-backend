 ...
 */
const getDriverTierDashboard = catchAsync(async (req: Request, res: Response) => {
  ...
  const levelMilestones = activeTiers.map((t) => {
    const isUnlocked = currentTier ? currentPoints >= t.requirements.pointsRequired : false;
    return {
      tierId: t._id,
      name: t.name,
      level: t.level,
  ...
  const todaysPoints = todaysPointsHistory.reduce((sum, h) => sum + h.points, 0);

  // 5. Get recent point history
  const recentPointHistory = await DriverPointHistory.find({ driverId: driverUserId })
    .sort({ createdAt: -1 })
    .limit(10);
  ...
  const destinationFiltersRemaining = Math.max(0, dailyLimit - filtersActivatedToday);
 ...
 */
const getDriverPointsHistory = catchAsync(async (req: Request, res: Response) => {
  const driverUserId = req.user.id;
  ...
  const historyQuery = new QueryBuilder(DriverPointHistory.find(filterQuery), req.query)
...
const getDriverPointRules = catchAsync(async (req: Request, res: Response) => {
  const rules = await PointRule.find({ status: "active" as any }).sort({ points: -1 });
 ...
 */
const getDriverTierBenefits = catchAsync(async (req: Request, res: Response) => {
  const driverUserId = req.user.id;

  const driver = await Driver.findOne({ userId: driverUserId }).populate("currentTier");
  if (!driver) {
    return sendResponse(res, {
      statusCode: StatusCodes.NOT_FOUND,
      success: false,
  ...
  const benefitsData = activeTiers.map((tier) => {
    const isUnlocked = currentTier ? (driver.currentPoints || 0) >= tier.requirements.pointsRequired : false;
    return {
      tierId: tier._id,
      name: tier.name,
      level: tier.level,
  ...
  const remainingPoints = driver.nextTier
    ? await Tier.findById(driver.nextTier).then((t) =>
        t ? Math.max(0, t.requirements.pointsRequired - (driver.currentPoints || 0)) : 0,
      )
    : 0;
