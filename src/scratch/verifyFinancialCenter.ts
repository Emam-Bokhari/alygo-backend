...
async function run() {
  try {
    ...
    rides.forEach((r: any, idx: number) => {
      console.log(`Ride #${idx + 1}: ID=${r._id}, totalFare=${r.fare?.total}, commission=${r.fare?.commission}`);
    });
    ...
    txs.forEach((t: any, idx: number) => {
      console.log(`Tx #${idx + 1}: ID=${t._id}, type=${t.transactionType}, amount=${t.amount}`);
    });
