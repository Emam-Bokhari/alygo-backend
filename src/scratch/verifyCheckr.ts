...
const stubCheckrService = () => {
  CheckrService.createCandidate = async (data: any) => {
    console.log("Mocked CheckrService.createCandidate called with:", JSON.stringify(data));
    return { id: "cand_mock_123" };
  };
  CheckrService.updateCandidate = async (id: string, data: any) => {
    console.log(`Mocked CheckrService.updateCandidate called for ${id} with:`, JSON.stringify(data));
    return { id };
  };
  CheckrService.createReport = async (candidateId: string, type: string) => {
    console.log(`Mocked CheckrService.createReport called for candidate ${candidateId} type ${type}`);
...
async function runVerification() {
  try {
    ...
    await DriverVerificationService.triggerMVRVerification(testDriver._id.toString());

    // Verify driver fields in DB
    const driverAfterMvr = await Driver.findById(testDriver._id);
    assert(driverAfterMvr?.checkrCandidateId === "cand_mock_123", "checkrCandidateId saved");
    assert(driverAfterMvr?.checkrMVRReportId === "rep_mvr_123", "checkrMVRReportId saved");
    assert(driverAfterMvr?.mvrStatus === VERIFICATION_STATUS.PENDING, "mvrStatus set to pending");

    // 4. Test Background Check initiation
    console.log("Testing initiateBackgroundCheck...");
    await DriverVerificationService.initiateBackgroundCheck(testUser._id.toString());
    const driverAfterBg = await Driver.findById(testDriver._id);
    assert(driverAfterBg?.checkrBackgroundReportId === "rep_bg_123", "checkrBackgroundReportId saved");
    assert(driverAfterBg?.backgroundCheckStatus === VERIFICATION_STATUS.PENDING, "backgroundCheckStatus set to pending");
    ...
    const key = config.checkr.signingKey || config.checkr.apiKey || "test_signing_key";
    const signature = crypto.createHmac("sha256", key).update(rawPayload).digest("hex");

    const computed = crypto.createHmac("sha256", key).update(rawPayload).digest("hex");
