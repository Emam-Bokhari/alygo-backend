...
async function runVerification() {
  try {
    ...
    assert(
      (createdUser as any).isDeleted === false,
      "Initial isDeleted is false.",
    );
    ...
    assert(
      (inactiveUser as any).isDeleted === false,
    ...
    assert(
      findNormal === null,
      "Soft-deleted user is not returned by normal findById query.",
    );

    // Verify state with deleted documents included
    const softDeletedUser = await User.findOne({ _id: userId }).setOptions({
      withDeleted: true,
    });
    assert(
      softDeletedUser !== null,
      "Soft-deleted user found using setOptions({ withDeleted: true }).",
    );
    assert(
      softDeletedUser?.status === STATUS.INACTIVE,
      "Status remains inactive.",
    );
    assert(
      (softDeletedUser as any).isDeleted === true,
    ...
    assert(
      restoredUser !== null,
      "Reactivated user is now visible to normal findById query.",
    );
    assert(
    ...
    assert(
      (restoredUser as any).isDeleted === false,
      "isDeleted is successfully restored to false.",
    );
    ...
    assert(
      (reActiveUser as any).isDeleted === false,
      "isDeleted remains false.",
    );
    ...
    assert(
      unrelatedUpdateResult !== null,
      "Unrelated update document returned.",
    );
    assert(
      unrelatedUpdateResult?.name === "New Reactivation Name",
      "Name updated successfully.",
    );
    assert(
      (unrelatedUpdateResult as any).isDeleted === true,
      "isDeleted remains true.",
    );
    assert(
      unrelatedUpdateResult?.status === STATUS.INACTIVE,
      "Status remains inactive.",
    );

    const normalFindUnrelated = await User.findById(userId);
    assert(
      normalFindUnrelated === null,
      "User still not visible to normal queries.",
    );
    ...
    assert(
      normalInactiveUser !== null,
      "User remains visible after active -> inactive transition.",
    );
    assert(
      normalInactiveUser?.status === STATUS.INACTIVE,
      "Status is inactive.",
    );
    assert(
      (normalInactiveUser as any).isDeleted === false,
      "isDeleted is still false.",
    );
