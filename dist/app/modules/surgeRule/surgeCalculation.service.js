...
exports.SurgeCalculationService = exports.getInterpolatedMultiplier = exports.isEventActive = exports.isHoliday = exports.isPeakHour = void 0;
 ...
 */
const isPeakHour = (date, peakHours) => __awaiter(void 0, void 0, void 0, function* () {
    if (!peakHours || peakHours.length === 0) {
        return false;
    }
    for (const peakHour of peakHours) {
        if ((0, timezoneHelper_1.isPeakHourActive)(peakHour.startTime, peakHour.endTime, peakHour.timezone, peakHour.applicableDays)) {
            return true;
        }
    }
    return false;
 ...
 */
const isHoliday = (date, holidays) => __awaiter(void 0, void 0, void 0, function* () {
    if (!holidays || holidays.length === 0) {
        return false;
    }
    for (const holiday of holidays) {
        if ((0, timezoneHelper_1.isHolidayActive)(holiday.startDate, holiday.endDate, holiday.timezone)) {
            return true;
        }
    }
    return false;
 ...
 */
const isEventActive = (date, events, serviceAreaId) => __awaiter(void 0, void 0, void 0, function* () {
    if (!events || events.length === 0) {
        return false;
    }
    for (const event of events) {
        // Check if current time is within event time range using timezone helper
        if (!(0, timezoneHelper_1.isEventActive)(event.startDateTime, event.endDateTime, event.timezone)) {
            continue;
        }
 ...
 */
const calculateDemand = (serviceAreaId) => __awaiter(void 0, void 0, void 0, function* () {
 ...
 */
const calculateSupply = (serviceAreaId) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    ...
    if (!serviceArea ||
        ...
        logger_1.logger.warn(`Active Service Area not found or location coordinates missing: ${serviceAreaId}`);
        return { totalDrivers: 0, availableDrivers: 0 };
    }
    const [centerLng, centerLat] = serviceArea.location.coordinates;
    const radiusKm = (_a = serviceArea.coverageRadiusKm) !== null && _a !== void 0 ? _a : 25;
    ...
    if (onlineDrivers.length > 0) {
        try {
            ...
            for (let i = 0; i < onlineDrivers.length; i++) {
                const result = (_b = matrix[i]) === null || _b === void 0 ? void 0 : _b[0];
            ...
            logger_1.logger.error(`[SurgeCalculationService] Error calculating drivers surge availability via Google: ${err}`);
            throw err;
        }
    }
 ...
 */
const selectApplicableRule = (rules, serviceAreaType, date, serviceAreaId) => __awaiter(void 0, void 0, void 0, function* () {
...
const getInterpolatedMultiplier = (ratio, minMultiplier, maxMultiplier) => {
    ...
    const normalizedRatio = (clampedRatio - lowerBoundRatio) / (upperBoundRatio - lowerBoundRatio);
 ...
 */
const calculateSurgeMultiplier = (serviceAreaId_1, ...args_1) => __awaiter(void 0, [serviceAreaId_1, ...args_1], void 0, function* (serviceAreaId, date = new Date()) {
    ...
    const activeRules = yield surgeRule_model_1.SurgeRule.find({ status: status_1.STATUS.ACTIVE });
    const matchedRule = yield selectApplicableRule(activeRules, serviceArea.type, date, serviceAreaId);
    if (!matchedRule) {
        return 1.0;
    }
    const { minMultiplier, maxMultiplier } = matchedRule;
    // Calculate smoothly interpolated multiplier
    return (0, exports.getInterpolatedMultiplier)(ratio, minMultiplier, maxMultiplier);
});
/**
 * Test surge calculation with detailed breakdown
 */
const testSurgeCalculation = (serviceAreaId_1, ...args_1) => __awaiter(void 0, [serviceAreaId_1, ...args_1], void 0, function* (serviceAreaId, date = new Date()) {
    const serviceArea = yield serviceArea_model_1.ServiceArea.findById(serviceAreaId);
    if (!serviceArea || serviceArea.status !== status_1.STATUS.ACTIVE) {
        throw new Error("Service area not found or inactive");
    }
    const activeRequests = yield calculateDemand(serviceAreaId);
    const { totalDrivers, availableDrivers } = yield calculateSupply(serviceAreaId);
    let ratio = 5.0;
    if (availableDrivers > 0) {
        ratio = activeRequests / availableDrivers;
    }
    ...
    const finalMultiplier = (0, exports.getInterpolatedMultiplier)(ratio, minMultiplier, maxMultiplier);
    return {
        demand: activeRequests,
        supply: { totalDrivers, availableDrivers },
