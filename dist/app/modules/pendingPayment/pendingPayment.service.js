 ...
 */
const getPendingPaymentsByUser = (userId) => __awaiter(void 0, void 0, void 0, function* () {
...
const payCancellationFeeNow = (userId, pendingPaymentId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (pendingPayment.userId.toString() !== userId) {
        throw new ApiErrors_1.default(403, "You do not have permission to pay this fee");
    }
    ...
    if (pendingPayment.stripeSessionId) {
        try {
            const existingSession = yield stripe_service_1.default.retrieveSession(pendingPayment.stripeSessionId);
            const amountCents = Math.round(pendingPayment.amount * 100);
            const isExpired = new Date(existingSession.expires_at * 1000) <= new Date();
            if (existingSession.status === "open" &&
                ...
                if (existingSession.status === "open") {
                    yield stripe_service_1.default.expireSession(pendingPayment.stripeSessionId);
                }
            }
        }
        catch (err) {
    ...
    if (pendingPayment.type === "cancellation_fee") {
        ...
        cancellationReasonId =
            ((_b = (_a = ride === null || ride === void 0 ? void 0 : ride.cancellation) === null || _a === void 0 ? void 0 : _a.cancellationReasonId) === null || _b === void 0 ? void 0 : _b.toString()) || "";
    }
    const metadata = {
    ...
    else if (pendingPayment.type === "driver_appreciation" &&
 ...
 */
const processCancellationFeePayment = (sessionId) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    const session = yield mongoose_1.default.startSession();
    session.startTransaction();
    try {
        // Find pending payment by session ID from metadata
        const stripeSession = yield stripe_service_1.default.retrieveSession(sessionId);
        const pendingPaymentId = (_a = stripeSession.metadata) === null || _a === void 0 ? void 0 : _a.pendingPaymentId;
        if (!pendingPaymentId) {
            yield session.abortTransaction();
            session.endSession();
            return;
        }
        const pendingPayment = yield pendingPayment_model_1.PendingPayment.findById(pendingPaymentId).session(session);
        if (!pendingPayment) {
            yield session.abortTransaction();
            session.endSession();
            return;
        ...
        const ride = yield ride_model_1.Ride.findById(pendingPayment.rideId).session(session);
        ...
        const driverCompensation = pendingPayment.driverCompensation !== undefined
 ...
 */
const payCancellationFeeWithWallet = (userId, pendingPaymentId) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const session = yield mongoose_1.default.startSession();
    session.startTransaction();
    try {
        ...
        if (pendingPayment.status !== "pending") {
            throw new ApiErrors_1.default(400, "This payment has already been processed");
        }
        ...
        if (isAppreciation) {
            ...
            if (pendingPayment.driverId) {
                ...
                if (driver) {
                    ...
                    driver.averageAppreciation = Number((newTotalAmount / newTotalReceived).toFixed(2));
            ...
            const ride = yield ride_model_1.Ride.findById(pendingPayment.rideId).session(session);
            // Credit driver compensation if applicable
            const walletDriverCompensation = pendingPayment.driverCompensation !== undefined
                ? pendingPayment.driverCompensation
 ...
 */
const processDriverAppreciationPayment = (sessionId) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const session = yield mongoose_1.default.startSession();
    session.startTransaction();
    try {
        ...
        yield transaction_service_1.TransactionService.createTransaction({
            userId: pendingPayment.userId,
            rideId: pendingPayment.rideId,
            transactionType: transaction_constant_1.TRANSACTION_TYPE.DRIVER_APPRECIATION,
            amount: pendingPayment.amount,
            paymentMethod: ride_constant_1.PAYMENT_METHOD.STRIPE,
            paymentStatus: ride_constant_1.PAYMENT_STATUS.PAID,
            stripeCheckoutSessionId: sessionId,
 ...
 */
const skipPendingPayment = (userId, pendingPaymentId) => __awaiter(void 0, void 0, void 0, function* () {
    const pendingPayment = yield pendingPayment_model_1.PendingPayment.findById(pendingPaymentId);
    if (!pendingPayment) {
        throw new ApiErrors_1.default(404, "Pending payment not found");
    }
    if (pendingPayment.userId.toString() !== userId) {
        throw new ApiErrors_1.default(403, "You do not have permission to skip this payment");
    }
    if (pendingPayment.status !== "pending") {
        throw new ApiErrors_1.default(400, "This payment has already been processed");
    }
    ...
    if (pendingPayment.type !== "driver_appreciation") {
        throw new ApiErrors_1.default(400, "Only driver appreciation payments can be skipped");
    }
