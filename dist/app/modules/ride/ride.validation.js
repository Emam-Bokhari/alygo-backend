...
const rideTypeSchema = zod_1.z
    .union([zod_1.z.nativeEnum(ride_constant_1.RIDE_TYPE), zod_1.z.literal("reservation")])
    .transform((val) => (val === "reservation" ? ride_constant_1.RIDE_TYPE.SCHEDULED : val));
const estimateRideZodSchema = zod_1.z.object({
    body: zod_1.z
        ...
        .refine((data) => {
        if (data.rideType === ride_constant_1.RIDE_TYPE.SCHEDULED && !data.serviceCategoryId) {
            return false;
        }
        return true;
    }, {
        message: "serviceCategoryId is required for Scheduled (Reservation) rides",
...
const requestRideZodSchema = zod_1.z.object({
    body: zod_1.z
        ...
        message: "scheduledAt is required and must be a future date/time for scheduled rides",
        path: ["scheduledAt"],
    })
        .refine((data) => {
        if (data.rideType === ride_constant_1.RIDE_TYPE.SCHEDULED && !data.serviceCategoryId) {
...
const verifyRideSecurityZodSchema = zod_1.z.object({
    body: zod_1.z
        .object({
        otp: zod_1.z.string().length(6, "OTP must be exactly 6 digits").optional(),
        ...
        message: "Either otp or phoneLastFourDigits must be provided for verification",
        path: ["otp"],
    }),
});
...
const driverRideHistoryQuerySchema = zod_1.z.object({
    query: zod_1.z.object({
        ...
        rideType: zod_1.z
            ...
            errorMap: () => ({
                message: "Invalid rideType provided. Must be 'instant' or 'scheduled'",
            }),
        })
            .optional(),
...
const userRideHistoryQuerySchema = zod_1.z.object({
    query: zod_1.z.object({
        status: zod_1.z.string().optional(),
        rideType: zod_1.z
            .enum(["instant", "scheduled"], {
...
const getMyReservationsQuerySchema = zod_1.z.object({
    query: zod_1.z.object({
        status: zod_1.z.string().optional(),
        reservationStatus: zod_1.z.string().optional(),
        page: zod_1.z.string().optional(),
        limit: zod_1.z.string().optional(),
    }),
});
