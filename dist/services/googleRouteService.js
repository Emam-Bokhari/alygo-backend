...
class GoogleRouteServiceClass {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (ctx && ctx.googleRouteCache) {
                if (ctx.googleRouteCache.has(key)) {
                    logger_1.logger.info(`[GoogleRouteService] Cache hit for key: ${key}`);
    ...
    fetchRawRoute(origin_1, destination_1) {
        return __awaiter(this, arguments, void 0, function* (origin, destination, stops = [], bypassCache) {
            ...
            const fetchFn = () => __awaiter(this, void 0, void 0, function* () {
                if (!this.apiKey) {
                    throw new Error("Google Maps API Key is missing. Cannot calculate route.");
                }
                ...
                logger_1.logger.info(`[GoogleRouteService] Fetching Directions API: Origin: ${originStr}, Dest: ${destStr}, Stops count: ${stops.length}${bypassCache ? " (Bypassing Cache)" : ""}`);
                const response = yield axios_1.default.get(url);
                const data = response.data;
                if (data.status !== "OK" || !data.routes || data.routes.length === 0) {
                    const errorMsg = data.error_message || `Directions API status: ${data.status}`;
                    logger_1.logger.error(`[GoogleRouteService] Directions API error: ${errorMsg}`);
    ...
    calculateRouteLegs(driverLocation_1, pickup_1, destination_1) {
        return __awaiter(this, arguments, void 0, function* (driverLocation, pickup, destination, stops = [], currentStopOrder, rideStatus, bypassCache) {
            const isRideStarted = (rideStatus === null || rideStatus === void 0 ? void 0 : rideStatus.toLowerCase()) === "started" ||
            ...
            if (!isRideStarted) {
                ...
                const data = yield this.fetchRawRoute(driverLocation, destination, waypoints, bypassCache);
                const route = data.routes[0];
                const legs = [];
                let totalDistanceMeters = 0;
                let totalDurationSeconds = 0;
                ...
                const activeStops = sortedStops.filter((stop) => !stop.isCompleted &&
                ...
                const data = yield this.fetchRawRoute(driverLocation, destination, activeWaypoints, bypassCache);
                const route = data.routes[0];
                const fullLegsDefs = [];
                ...
                const activeStopOrder = currentStopOrder !== undefined ? currentStopOrder : -1;
                ...
                for (let idx = 0; idx < fullLegsDefs.length; idx++) {
                    ...
                    if (idx < activeLegIndex) {
                        // Completed/past leg
                        distanceKm = this.getHaversineDistanceKm(def.fromCoords, def.toCoords);
    ...
    calculateDistanceMatrix(origins, destinations) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return this.withCache(cacheKey, () => __awaiter(this, void 0, void 0, function* () {
                if (!this.apiKey) {
                    throw new Error("Google Maps API Key is missing. Cannot calculate distance matrix.");
                }
                const originChunks = [];
                const chunkSize = 25;
                ...
                for (const chunk of originChunks) {
                    ...
                    logger_1.logger.info(`[GoogleRouteService] Fetching Distance Matrix API: Origins count: ${chunk.length}, Destinations count: ${destinationsStrList.length}`);
                    const response = yield axios_1.default.get(url);
                    const data = response.data;
                    if (data.status !== "OK") {
                        const errorMsg = data.error_message || `Distance Matrix API status: ${data.status}`;
                        logger_1.logger.error(`[GoogleRouteService] Distance Matrix API error: ${errorMsg}`);
                        throw new Error(errorMsg);
                    }
                    allRows.push(...data.rows);
                }
                return allRows.map((row) => row.elements.map((elem) => {
                    if (elem.status !== "OK") {
                        return {
                            distanceKm: 9999,
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return this.withCache(cacheKey, () => __awaiter(this, void 0, void 0, function* () {
                ...
                if (data.status !== "OK" || !data.results || data.results.length === 0) {
                    throw new Error(data.error_message || `Geocoding API status: ${data.status}`);
                }
    ...
    reverseGeocode(lat, lng) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.apiKey) {
                throw new Error("Google Maps API Key is missing. Cannot reverse geocode.");
            }
            ...
            return this.withCache(cacheKey, () => __awaiter(this, void 0, void 0, function* () {
                ...
                const data = response.data;
                if (data.status !== "OK" || !data.results || data.results.length === 0) {
                    throw new Error(data.error_message || `Reverse Geocoding API status: ${data.status}`);
                }
    ...
    getRoadDistance(origin, destination) {
        return __awaiter(this, void 0, void 0, function* () {
            const matrix = yield this.calculateDistanceMatrix([origin], [destination]);
    ...
    resolveCurrentTrackingState(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const transitions = [];
            let currentStopOrder = (_a = tracking.currentStopOrder) !== null && _a !== void 0 ? _a : -1;
            ...
            let maxIterations = 3;
            let iteration = 0;
            let currentTargetDetails = getTargetDetails(rideStatus, ride.pickup, ride.destination, ride.stops, currentStopOrder);
            ...
            while (iteration < maxIterations) {
                ...
                routeLegsResult = yield this.calculateRouteLegs(driverLocation, pickupCoord, destCoord, ride.stops, currentStopOrder, rideStatus, true);
                ...
                if (activeLegs.length > 1) {
                    throw new Error(`Inconsistent tracking state: Multiple active legs detected (${activeLegs.length})`);
                }
                ...
                if (activeLegs.length === 0) {
                    if (iteration < maxIterations) {
                        ...
                        if (routeLegs.length > 0) {
                            ...
                            throw new Error("Inconsistent tracking state: No route legs calculated.");
                        }
                    }
                }
                else {
                ...
                // 3. STARTED -> Stop completed
                else if (rideStatus === ride_constant_1.RIDE_STATUS.STARTED &&
                    ride.stops &&
                    ride.stops.length > 0) {
                    ...
                    if (nextStop && !nextStop.isCompleted) {
                        if (currentTargetDetails.targetType === "stop" &&
                ...
                currentTargetDetails = getTargetDetails(rideStatus, ride.pickup, ride.destination, ride.stops, currentStopOrder);
                if (!transitionDetected) {
                    break;
                }
            }
...
const getTargetDetails = (rideStatus, pickup, destination, stops, currentStopOrder) => {
