  ...
  io.on("connection", (socket) =>
    __awaiter(void 0, void 0, void 0, function* () {
      ...
      logger_1.logger.info(
        colors_1.default.blue("A User connected to Socket.IO"),
      );
      // Attempt authentication via token in query or auth object
      const token =
        ((_a = socket.handshake.query) === null || _a === void 0
          ? void 0
          : _a.token) ||
      ...
      socket.on("register", (data) => {
        if (data === null || data === void 0 ? void 0 : data.userId) {
          ...
          socketMap.set(userId, socket);
          socket.data = Object.assign(Object.assign({}, socket.data), {
            userId,
          });
          logger_1.logger.info(
            colors_1.default.green(
              `Socket manually registered User ID: ${userId}`,
            ),
          );
        }
      });
      ...
      socket.on("driver-location-update", (data) =>
        __awaiter(void 0, void 0, void 0, function* () {
          ...
          try {
            logger_1.logger.info(
              "Driver location update received via WebSocket",
            );
            const userId =
              (_a = socket.data) === null || _a === void 0 ? void 0 : _a.userId;
            if (!userId) {
      ...
      socket.on("user-location-update", (data) =>
        __awaiter(void 0, void 0, void 0, function* () {
          ...
          try {
            ...
            logger_1.logger.error(
              "Error handling user-location-update: " +
                (error.message || error),
            );
          }
        }),
      ...
      socket.on("disconnect", (reason) => {
        ...
        if (userId) {
          ...
          logger_1.logger.info(
            colors_1.default.red(
              `User ${userId} disconnected. Reason: ${reason}`,
            ),
          );
        } else {
          logger_1.logger.info(
            colors_1.default.red(`A user disconnect. Reason: ${reason}`),
          );
        }
      });
    }),
...
const sendToUser = (userId, event, data) => {
  ...
  if (clientSocket) {
    ...
    logger_1.logger.info(
      colors_1.default.green(`Socket event '${event}' sent to user: ${key}`),
    );
    return true;
  }
  logger_1.logger.warn(
    colors_1.default.yellow(
      `Socket event '${event}' FAILED - user ${key} not connected`,
    ),
  );
  return false;
};
