 ...
 */
const calculateDriverSearchTiming = (ride) => __awaiter(void 0, void 0, void 0, function* () {
    const now = new Date();
    const requestedAt = new Date(ride.requestedAt);
    const systemConfig = yield (0, systemConfigHelper_1.getSystemConfig)();
    const rideRequestLifetimeSeconds = systemConfig.driverMatching.rideRequestLifetimeSeconds;
    // Calculate elapsed time in seconds
    const elapsedMs = now.getTime() - requestedAt.getTime();
    const elapsedSeconds = Math.floor(elapsedMs / 1000);
    // Calculate remaining time
    const remainingSeconds = Math.max(0, rideRequestLifetimeSeconds - elapsedSeconds);
    // Calculate progress percentage
    const progressPercentage = Math.min(100, (elapsedSeconds / rideRequestLifetimeSeconds) * 100);
    ...
    const isExpired = ride.status === ride_constant_1.RIDE_STATUS.EXPIRED ||
